...
{
    return seL4_ARM_PageDirectory_Invalidate_Data(dma.vspace, addr, addr + size);
}

seL4_Error sos_dma_cache_clean(uintptr_t addr, size_t size)
{
    return seL4_ARM_PageDirectory_Clean_Data(dma.vspace, addr, addr + size);
}

seL4_Error sos_dma_cache_clean_invalidate(uintptr_t addr, size_t size)
{
    return seL4_ARM_PageDirectory_CleanInvalidate_Data(dma.vspace, addr, addr + size);
}
